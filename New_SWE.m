...
FoldPath='E:\RADI\data\201605\_\_\';
Fold_output_path='E:\RADI\data\output\';
for yyyy = 2010:2017
    for mm = 1:12
        for dd = 1:31
            orbit=['A','D'];
            ...
            for orbt_indx = 1:2
                for sat_index=1:2
                    ...
                    if Num_image==0
                        ...
                        SWE_proj_max1 = zeros(2841,2841)*NaN;
...
%                             EO_BT_info(LandCover==0)=NaN;   %water
                            ...
                            indata_mask = double(LandSeaMask);
                            lat = double(Lati_info);
                            lon = double(Long_info);
                            
                            indata1=indata;
                            indata1(LandSeaMask==0)=0;
                            indata1(isnan(LandSeaMask))=NaN;
                            
                            %4.Map display and output
                            surfm(lat, lon, indata1);
                            ...
                            title([fn_output_title,'CD'],'FontSize',14,'FontWeight','Bold');
                            %pcolorm(y, x, indata);
                            ...
                            SWE_proj_max=Image_projection(indata,lat,lon,SWE_proj_max);
                            SWE_proj_max1=Image_projection(indata_mask,lat,lon,SWE_proj_max1);
                        end


                        % Output
                        % SWE_integ=SWE_proj(:,:,2)./(Num_image-NaN_index);
                        SWE_integ=SWE_proj_max;
 
                        % SWE_integ(SWE_integ<0)=0;
                        image_output=[Fold_output_path,num2str(yyyy),num2str(mm,'%02d'),...
                            num2str(dd,'%02d'),'_',satllt(sat_index),'_',orbit(orbt_indx),'_','image','.tif'];
                        SWE_integ(SWE_proj_max1==0)=0;
                        SWE_integ(isnan(SWE_proj_max1))=NaN;
                        SWE_integ(isnan(SWE_integ))=999;
                        
                        SWE_integ = uint16(SWE_integ);
                        t = Tiff(image_output,'w');
                            tagstruct.ImageLength = size(SWE_proj,1);
                            tagstruct.ImageWidth = size(SWE_proj,2);
